 ...
 */
package net.theairblow.lunarium.gui.window;
...
import net.theairblow.lunarium.utils.ColorHelper;
...
public class Window {
	...
	public Window(int x1, int y1, int x2, int y2, String title, ItemStack icon) {
		this(x1, y1, x2, y2, title, icon, false, false);
	}

	public Window(int x1, int y1, int x2, int y2, String title, ItemStack icon, boolean closed, boolean uncloasable) {
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x2;
		this.y2 = y2;
	...
	protected void drawBar(MatrixStack matrix, int mX, int mY, TextRenderer textRend) {
		...
		fillGradient(matrix, x1 + 2, y1 + 2, x2 - 2, y1 + 12, (selected ? ColorHelper.getIntFromColor(179, 179, 179) : ColorHelper.getIntFromColor(179, 179, 179)), (selected ? ColorHelper.getIntFromColor(140, 148, 148) : ColorHelper.getIntFromColor(170, 172, 172)));

		/* buttons */
		if (!this.uncloasable) {
			fillGrey(matrix, x2 - 12, y1 + 3, x2 - 4, y1 + 11);
			textRend.draw(matrix, "x", x2 - 11, y1 + 2, ColorHelper.getIntFromColor(102, 102, 102));
		}
	}
	...
	public void fillGrey(MatrixStack matrix, int x1, int y1, int x2, int y2) {
		DrawableHelper.fill(matrix, x1, y1, x2 - 1, y2 - 1, ColorHelper.getIntFromColor(166, 166, 166));
		DrawableHelper.fill(matrix, x1 + 1, y1 + 1, x2, y2, ColorHelper.getIntFromColor(102, 102, 102));
		DrawableHelper.fill(matrix, x1 + 1, y1 + 1, x2 - 1, y2 - 1, ColorHelper.getIntFromColor(82, 82, 82));
	}
