...
import net.theairblow.lunarium.event.events.EventReadPacket;
...
import net.theairblow.lunarium.module.mods.CustomChat.CustomFont.CharMap;
				...
				for (PlayerEntity e : mc.world.getPlayers()) {
					...
					if (mc.player.distanceTo(e) < 12 && msg.contains(e.getName().asString())
						...
						if (getSetting(4).asMode().mode == 1) {
							mc.player.sendChatMessage(e.getName().asString() + " Just got EZed using the power of LunariumClient " + LunariumClient.VERSION);
						} else {
							mc.player.sendChatMessage("GG, " + e.getName().asString() + ", but LunariumClient " + LunariumClient.VERSION + " is ontop!");
						}
					}
				}
			}
