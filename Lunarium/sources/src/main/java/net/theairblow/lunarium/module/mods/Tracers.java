...
import net.theairblow.lunarium.utils.EntityUtils;
	...
	public void onRender(EventWorldRender event) {
		...
		for (Entity e : mc.world.getEntities()) {
			...
			if (e instanceof PlayerEntity && e != mc.player && e != mc.cameraEntity && getSetting(0).asToggle().state) {
				col = getSetting(0).asToggle().getChild(LunariumClient.friendMang.has(e.getName().asString()) ? 1 : 0).asColor().getRGBFloat();
