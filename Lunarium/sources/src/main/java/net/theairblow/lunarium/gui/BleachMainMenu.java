...
import net.theairblow.lunarium.module.mods.UI;
import net.theairblow.lunarium.update.VersionManager;
...
public class BleachMainMenu extends AbstractWindowScreen {
	...
	public static String splash = "why";
	...
	public void init() {
		...
		windows.add(new Window(width / 8 + 15,
				...
				height / 8 + 15 + (height - height / 2), "Login Manager", new ItemStack(Items.PAPER), true, false));
		windows.add(new Window(width / 8 + 30,
				height / 8 + 30,
				width / 8 + 30 + (width - width / 2),
				height / 8 + 30 + (height - height / 2), "Accounts", new ItemStack(Items.WRITABLE_BOOK), true, false));
	...
	public void onRenderWindow(MatrixStack matrix, int window, int mX, int mY) {
		...
		if (window == 0) {
			...
			drawCenteredString(matrix, this.textRenderer, LunariumClient.VERSION, (int) ((x + w / 2) / 1.5), (int) ((y + h / 4 + 6) / 1.5), 0xffc050);
